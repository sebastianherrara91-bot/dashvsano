...
## 7. Changelog v2.1 — Refactorización Crítica de Latencia SQL (Partition Pruning & Hash Joins)
Se detectó un cuello de botella grave donde la consulta demoraba hasta 30 segundos en el servidor LXC debido al uso de planes de ejecución genéricos (Generic Plans) provocados por los parámetros de psycopg2 en las fechas, y decisiones erróneas del motor relacional al estimar costos con el CTE `Valid_Marca_Tipo`.

Se realizaron las siguientes intervenciones clave en `api.py`:
1. **Inyección directa de fechas (Partition Pruning):** Para evitar que el servidor evaluara los años enteros y descartara el particionamiento de tablas (Partition Pruning), las fechas `fecha_inicio` y `fecha_fin` se pasaron a formato cadena nativa usando f-strings seguros. Esto permitió a PostgreSQL podar y descartar más del 80% de las tablas históricas antes de arrancar.
2. **Deshabilitación forzada del Nested Loop (`enable_nestloop = off`):** Al unir `Valid_Marca_Tipo` con las masivas tablas `dwh_stock` y `dwh_ventas`, PostgreSQL se equivocaba estimando cardinalidades y elegía un bucle anidado que se repetía 17,000 veces (tomando decenas de segundos). Se inyectó `SET LOCAL enable_nestloop = off;` dentro del cursor, forzando un **Hash Join** y un Bitmap Heap Scan. 
3. **Escalamiento del `work_mem`:** Sabiendo que el contenedor tiene 3GB de RAM libres, se aumentó el límite dinámico a `1GB` temporalmente por transacción para soportar los grandes Hashes generados por el paso anterior.
4. **Filtros CTE:** Se movieron los filtros `marca` y `tipo_programa` directamente al interior del CTE `Valid_Marca_Tipo`, purificando el join inicial.

**Resultado de v2.1:** Los tiempos cayeron de 28-30 segundos a apenas **0.7 segundos** procesando la data concurrente interanual, resolviendo el problema de lentitud definitiva.
